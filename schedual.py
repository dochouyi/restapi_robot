...
def place_limit_then_market_entry(client: FreqtradeClient, pair: str, stake_amount: float, wait_seconds: float = 2):
    """
    先限价入场，等待 wait_seconds，如未成交则撤销并用市价入场。
    """
    side = "long"
    # 1) 限价入场
    flag=client.force_enter_order(pair=pair, side=side, order_type="limit", stake_amount=stake_amount, leverage=10)
    if flag==True:
        # # 2) 等待成交
        ...
        print("出现问题，入场失败，可能是资金量不够")
    client.previous_trade_stake_amount = client.get_trade_stake_amount()
    print('当前持仓：',client.previous_trade_stake_amount)

def place_limit_then_market_exit_10_percent(client: FreqtradeClient, amount: None, wait_seconds: float = 5):
